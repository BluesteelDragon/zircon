...
import { ExecutionAction } from "Class/zircon-configuration-builder";
...
class Example {
	private readonly _logger = Log.ForContext(Example);
	public example(): void {
	...
	new ZirconConfigurationBuilder()
		...
		.AddNamespace(
			new ZirconNamespaceBuilder("example")
				.AddHelpFunction()
				.AddFunction(
					new ZirconFunctionBuilder("print_b")
						.AddVariadicArgument("string")
						.Bind((context, ...args) => {
				...
				.AddFunction(
					new ZirconFunctionBuilder("with_unions")
						...
						.Bind((context, ...strOrNumber) => {
							context.LogInfo("Got a {$Value}", strOrNumber);
						}),
				)
				.Build(),
		...
		.AddFunction(
			new ZirconFunctionBuilder("enum_test").AddArgument("ZrEnum").Bind((_, value) => {
				Log.Info(
					...
					value.getEnumName(),
					value.getItems().map(item => item.getName()),
				);
			}),
			[ZirconDefaultGroup.User],
		)
		.AddFunction(
			new ZirconFunctionBuilder("array_test")
				.AddArrayArgument(["string"])
				.Bind((context, myArray) => {
		...
		.AddHook("BeforeExecute", context => ExecutionAction.Execute)
